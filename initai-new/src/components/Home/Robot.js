...
export default function Robot() {
  return (
    <Canvas gl={{ logarithmicDepthBuffer: true }} shadows camera={{ position: [-15, 0, 10], fov: 25 }} className="robot ">
...
function Kamdo(props) {
  ...
  const stripe = useRef()
  const light = useRef()
  const { nodes, materials } = useGLTF('/robot_drone.glb')
  ...
  materials.Body.castShadow = false;
  materials.Head.castShadow = false;
  materials.Head.receiveShadow = false;
...
useGLTF.preload('/robot_drone.glb')
